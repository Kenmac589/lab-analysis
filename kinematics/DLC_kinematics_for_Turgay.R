...
data_path <- "Z:\\Turgay\\SCR\\SCR M2 WT\\SCRM2 20200130\\SCRM2 20200130\\"
...
calibration_factors <- c(horiz=20,
                         vert=25,
                         diag=sqrt(20^2+25^2))

KINEMATICS <- list.files(data_path, pattern="DLC.*.csv")

mouse_n <- length(unique(gsub("_.*", "", KINEMATICS)))
mouse_data <- data.frame(Code=unique(gsub("_.*", "", KINEMATICS)),
                         Femur=numeric(length=mouse_n),
                         Tibia=numeric(length=mouse_n))
mouse_data[] <- lapply(mouse_data, as.character)

qq <- 1
while (!is.na(qq)) {
    cat("\nI found ", mouse_n, " different mice in the data set",
        "\n\nDo you want to input femur and tibia length now (type 'y' for 'yes' or 'n' for 'no')?", sep="")
    qq <- readline()
...
if (qq=="y") {
    ...
    for (trial in 1:mouse_n) {
        
        # Prompt for femur length
        pp <- 0.2
        
        while (pp<1) {
            pp <- readline(paste0("Femur length in [mm] for mouse ", mouse_data$Code[trial], ": "))
            ...
            if (grepl("^$", pp)) {
                ...
                pp <- 1000
            } else if (as.numeric(pp)>25){
                pp <- -1
            }  else if (as.numeric(pp)<8){
                pp <- -1
            }
        }
        mouse_data$Femur[trial] <- pp
        
        # Prompt for tibia length
        pp <- 0.2
        
        while (pp<1) {
            pp <- readline(paste0("Tibia length in [mm] for mouse ", mouse_data$Code[trial], ": "))
        ...
        mouse_data$Tibia[trial] <- pp
    }
    
    mouse_data$Femur <- as.numeric(mouse_data$Femur)
    mouse_data$Tibia <- as.numeric(mouse_data$Tibia)
    
    write.csv(mouse_data,
              file=paste0(data_path, "mouse_data.csv"),
              quote=F, row.names=F)
    
    cat("\n\nThanks, all the femur and tibia lengths correctly saved!",
        "\n\nStart reading and calibrating files", sep="")
    
    dir.create(file.path(data_path, "Analysis"), showWarnings=F)
    
    # Parametres for the Hampel filter (outlier checking)
    # Window length in frames (2*nf+1)
    nf <- 10
    # Thresholds for the three consecutive filterings
    ...
    for (trial in 1:length(KINEMATICS)) {
        ...
        for (marker in markers) {
            ...
            data_x <- pracma::hampel(data_x, nf, sd1)$y
            data_x <- pracma::hampel(data_x, nf, sd2)$y
            data_x <- pracma::hampel(data_x, nf, sd3)$y
            
            data_y <- pracma::hampel(data_y, nf, sd1)$y
            data_y <- pracma::hampel(data_y, nf, sd2)$y
            data_y <- pracma::hampel(data_y, nf, sd3)$y
        ...
        dist25 <- sqrt((calib_set$Calib2_x-calib_set$Calib5_x)^2+(calib_set$Calib2_y-calib_set$Calib5_y)^2)
        dist36 <- sqrt((calib_set$Calib3_x-calib_set$Calib6_x)^2+(calib_set$Calib3_y-calib_set$Calib6_y)^2)
        
        # Diagonal distances
        dist15 <- sqrt((calib_set$Calib1_x-calib_set$Calib5_x)^2+(calib_set$Calib1_y-calib_set$Calib5_y)^2)
        dist24 <- sqrt((calib_set$Calib2_x-calib_set$Calib4_x)^2+(calib_set$Calib2_y-calib_set$Calib4_y)^2)
        dist26 <- sqrt((calib_set$Calib2_x-calib_set$Calib6_x)^2+(calib_set$Calib2_y-calib_set$Calib6_y)^2)
        dist35 <- sqrt((calib_set$Calib3_x-calib_set$Calib5_x)^2+(calib_set$Calib3_y-calib_set$Calib5_y)^2)
        
        distances <- data.frame(dist12, dist23, dist45, dist56,
                                dist14, dist25, dist36,
                                dist15, dist24, dist26, dist35)
        
        # Frame-by-frame calibration
        temp <- numeric()
        for (frame in 1:nrow(distances)) {
            frame_distances <- c(horiz=mean(as.numeric(distances[frame, 1:4])),
                                 vert=mean(as.numeric(distances[frame, 5:7])),
                                 diag=mean(as.numeric(distances[frame, 8:11])))
            
            calibration_matrix <- data.frame(cal_facts=calibration_factors,
                                             pixels=frame_distances,
                                             cal_pixel=calibration_factors/frame_distances)
            
            calibration_factor <- mean(calibration_matrix$cal_pixel)
            temp[frame] <- calibration_factor
            kin_data[frame, ] <- kin_data[frame, ]*calibration_factor
        }
        
        # Remove calibration columns
        ...
        write.csv(kin_data,
                  file=paste0(data_path, "Analysis\\",
                              gsub("DeepCut.*", "", KINEMATICS[trial]), "_for_Spike2.csv"),
                  quote=F, row.names=F)
    }
} else {
    cat("\nThe analysis cannot continue without femur and tibia lengths!", sep="")
}
