...
def main():
    ...
    #     "./egr3-10-perturbation-xcom.txt", delimiter=",", header=0
    # )
    # egr3_10sindf = pd.read_csv("./egr3-10-sinus-xcom.txt", delimiter=",", header=0)

    # Some things to set for plotting/saving
    manual_analysis = False
    lmos_filename = "./egr3_8per_lmos.csv"
    rmos_filename = "./egr3_8per_rmos.csv"
    figure_title = "Measurement of Stability For Egr3 KO M8 with Perturbation"

    # Grabbing individual channels
    xcom = egr3_8perdf["v1 xCoM"].to_numpy(dtype=float)
    leftcop = egr3_8perdf["v3 L COP"].to_numpy(dtype=float)
    rightcop = egr3_8perdf["v2 R COP"].to_numpy(dtype=float)
    left_DS = egr3_8perdf["59 left DS"].to_numpy(dtype=float)
    right_DS = egr3_8perdf["60 right DS"].to_numpy(dtype=float)
