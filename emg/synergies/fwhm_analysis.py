...
for i in range(0, synergy_selection):
    ...
    for j in range(0, len(conditions_wt_primitives)):
        ...
        for k in range(0, len(fwhm_list)):
            fwhm_entry = [
                [condition_tag, perturbation_state_tag, synergy_tag, fwhm_list[k]]
            ]
            fwhm_df = fwhm_df._append(
                pd.DataFrame(
                    fwhm_entry,
                    columns=["Condition", "Perturbation State", "Synergy", "FWHM"],
                ),
                ignore_index=True,
            )
...
conditions_name = ["PreDTX", "PreDTX", "PostDTX", "PostDTX"]
