...
for i in range(14):
    if i < 10:
        ...
        print(f"Hip height for video {video}: {hip_h}")
...
platform_hip_heights = np.array([])
for i in range(14):
    if i < 10:
        video = f"{i}"
        df, bodyparts, scorer = dlck.load_data(
            f"./platform_recordings/2024-04-24_00000{video}DLC_resnet50_SN_7 Pre SN.E-SCRApr25shuffle1_800000.h5"
        )
        ...
        platform_hip_heights = np.append(platform_hip_heights, hip_h)
    else:
        video = f"{i}"
        df, bodyparts, scorer = dlck.load_data(
            f"./platform_recordings/2024-04-24_0000{i}DLC_resnet50_SN_7 Pre SN.E-SCRApr25shuffle1_800000.h5"
        )
...
print(f"Platform average {np.mean(platform_hip_heights)}")
# legend = ["hip_y", "toe_y"]
